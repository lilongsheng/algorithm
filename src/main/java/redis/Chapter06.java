 ...
 *
 * 分布式锁
 * key "locked_name" value "uuid"
 * setnx  uuid 来确保每个客户端持有自己创建的锁，释放自己的锁
 * 锁的粒度影响性能
 *
 *
 * 带有超时特性的锁（避免在客户端崩溃后锁一直持有不被释放）
 *
 *
 *
 *
...
//        testFileDistribution(conn);
    ...
    public void testAddUpdateContact(Jedis conn) {
        ...
        log.info("Let's add a few contacts...");
        for (int i = 0; i < 10; i++){
            addUpdateContact(conn, "user", "contact-" + ((int)Math.floor(i / 3)) + '-' + i);
        }
        log.info("Current recently contacted contacts");
        ...
        log.info("Let's pull one of the older ones up to the front");
        addUpdateContact(conn, "user", "contact-1-4");
        contacts = conn.lrange("recent:user", 0, 2);
        log.info("New top-3 contacts:");
        for(String contact : contacts){
            log.info("  " + contact);
        }
        assert "contact-1-4".equals(contacts.get(0));

        log.info("Let's remove a contact...");
        ...
        log.info("And let's finally autocomplete on ");
    ...
    public void testAddressBookAutocomplete(Jedis conn) {
        ...
        log.info("the start/end range of 'abc' is: " +
            Arrays.toString(findPrefixRange("abc")));

        log.info("Let's add a few people to the guild");
        for (String name : new String[]{"jeff", "jenny", "jack", "jennifer"}){
            joinGuild(conn, "test", name);
        }
        log.info("now let's try to find users with names starting with 'je':");
        Set<String> r = autocompleteOnPrefix(conn, "test", "je");
        log.info("{}",r);
        assert r.size() == 3;

        log.info("jeff just left to join a different guild...");
        ...
        throws InterruptedException
    {
        ...
        log.info("Getting an initial lock...");
        assert acquireLockWithTimeout(conn, "testlock", 1000, 1000) != null;
        log.info("Got it!");
        log.info("Trying to get it again without releasing the first one...");
        assert acquireLockWithTimeout(conn, "testlock", 10, 1000) == null;
        log.info("Failed to get it!");

        log.info("Waiting for the lock to timeout...");
        Thread.sleep(2000);
        log.info("Getting the lock again...");
        ...
        log.info("Released it...");

        log.info("Acquiring it again...");
        assert acquireLockWithTimeout(conn, "testlock", 1000, 1000) != null;
        log.info("Got it!");
        conn.del("lock:testlock");
    }
        ...
        throws InterruptedException
    {
        ...
        log.info("Getting 3 initial semaphores with a limit of 3...");
        for (int i = 0; i < 3; i++) {
            assert acquireFairSemaphore(conn, "testsem", 3, 1000) != null;
        }
        log.info("Done!");
        log.info("Getting one more that should fail...");
        assert acquireFairSemaphore(conn, "testsem", 3, 1000) == null;
        log.info("Couldn't get it!");

        log.info("Lets's wait for some of them to time out");
        Thread.sleep(2000);
        log.info("Can we get one?");
        String id = acquireFairSemaphore(conn, "testsem", 3, 1000);
        assert id != null;
        log.info("Got one!");
        log.info("Let's release it...");
        assert releaseFairSemaphore(conn, "testsem", id);
        log.info("Released!");
        log.info("And let's make sure we can get 3 more!");
        for (int i = 0; i < 3; i++) {
            assert acquireFairSemaphore(conn, "testsem", 3, 1000) != null;
        }
        log.info("We got them!");
        conn.del("testsem", "testsem:owner", "testsem:counter");
    }
        ...
        throws InterruptedException
    {
        ...
        log.info("Let's start some regular and delayed tasks...");
        ...
        log.info("How many non-delayed tasks are there (should be 2)? " + r);
        assert r == 2;

        log.info("Let's start up a thread to bring those delayed tasks back...");
        PollQueueThread thread = new PollQueueThread();
        thread.start();
        log.info("Started.");
        log.info("Let's wait for those tasks to be prepared...");
        Thread.sleep(2000);
        thread.quit();
        thread.join();
        r = conn.llen("queue:tqueue");
        log.info("Waiting is over, how many tasks do we have (should be 4)? " + r);
        assert r == 4;
        conn.del("queue:tqueue", "delayed:");
    }
    ...
    public void testMultiRecipientMessaging(Jedis conn) {
        ...
        log.info("Let's create a new chat session with some recipients...");
        Set<String> recipients = new HashSet<String>();
        recipients.add("jeff");
        recipients.add("jenny");
        String chatId = createChat(conn, "joe", recipients, "message 1");
        log.info("Now let's send a few messages...");
        for (int i = 2; i < 5; i++){
            sendMessage(conn, chatId, "joe", "message " + i);
        }

        log.info("And let's get the messages that are waiting for jeff and jenny...");
        List<ChatMessages> r1 = fetchPendingMessages(conn, "jeff");
        List<ChatMessages> r2 = fetchPendingMessages(conn, "jenny");
        log.info("They are the same? " + r1.equals(r2));
        assert r1.equals(r2);
        log.info("Those messages are:");
        for(ChatMessages chat : r1){
            log.info("  chatId: " + chat.chatId);
            log.info("    messages:");
        ...
        throws InterruptedException, IOException
    {
        ...
        log.info("Creating some temporary 'log' files...");
        ...
        long size = f3.length();
        log.info("Done.");
        log.info("Starting up a thread to copy logs to redis...");
        File path = f1.getParentFile();
        CopyLogsThread thread = new CopyLogsThread(path, "test:", 1, size);
        thread.start();

        log.info("Let's pause to let some logs get copied to Redis...");
        Thread.sleep(250);
        log.info("Okay, the logs should be ready. Let's process them!");

        log.info("Files should have 1, 100, and 1000 lines");
        TestCallback callback = new TestCallback();
        processLogsFromRedis(conn, "0", callback);
        log.info(Arrays.toString(callback.counts.toArray(new Integer[0])));
        assert callback.counts.get(0) == 1;
        assert callback.counts.get(1) == 100;
        assert callback.counts.get(2) == 1000;

        log.info("Let's wait for the copy thread to finish cleaning up...");
        thread.join();
        log.info("Done cleaning out Redis!");
    ...
    public List<ChatMessages> fetchPendingMessages(Jedis conn, String recipient) {
        ...
        while (seenIterator.hasNext()){
            ...
            for (String messageJson : messageStrings){
                ...
                Map<String,Object> message = (Map<String,Object>) JSON.parseObject(
                    messageJson,Map.class);
